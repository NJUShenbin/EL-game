...
import cn.edu.nju.components.ExitButton.ExitListener;

public class ExitWhite extends JLabel{
	private static final long serialVersionUID = 1L;
	Image next = new ImageIcon("images/whiteExit.png").getImage();
	Boolean mouseContained = false;
	Boolean mouseClicked = false;
	Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
public ExitWhite(){
	
	this.setBounds(screenSize.width-55,20 , 50, 50);
...
public void paintComponent(Graphics g) {
	...
	g2d.drawImage(next, -2,2, this.getWidth()-5,this.getWidth()-5,null);
		...
		this.setBounds(screenSize.width-60, 15, 60, 60);
